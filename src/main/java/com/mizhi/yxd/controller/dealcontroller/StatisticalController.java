...
public class StatisticalController {
    ...
    @PostMapping("/learningPeriodWithSubsidize")
    public Object learningPeriodWithSubsidize(StatisticQueryVo statisticQueryVo, HttpSession httpSession) {
        log.info("statistic learningPeriodWithSubsidize query vo message:{}", JSON.toJSONString(statisticQueryVo));
        statisticQueryVo.setAccount((String) httpSession.getAttribute("account"));
        ...
        Layui l = Layui.data(learningPeriodWithSubsidizeVos.size(), learningPeriodWithSubsidizeVos);
        return JSON.toJSON(l);
    }
    ...
    public void schoolPeriodInMizhiNotBuildCard(@RequestParam String semester, HttpSession httpSession, HttpServletResponse response) throws IOException {
        ...
        if (CollectionUtils.isNotEmpty(learningPeriodInMizhiVos)) {
            List<SchoolPeriodInMizhiVo> schoolPeriodInMizhiVos = BeanUtils.copyProperties(learningPeriodInMizhiVos, SchoolPeriodInMizhiVo.class);
            ExcelUtils.exportExcel(schoolPeriodInMizhiVos, null, "统计信息", SchoolPeriodInMizhiVo.class, "米脂县就读分学校分学段资助汇总", true, response);
        }
    }

    @GetMapping("/learningPeriodWithSubsidize/export")
